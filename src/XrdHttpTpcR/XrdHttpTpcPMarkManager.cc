...
PMarkManager::PMarkManager(XrdHttpExtReq & req, TPCR::TpcType tpcType) : mPmark(req.pmark), mReq(req), mTransferWillStart(false), mTpcType(tpcType) {}
...
void PMarkManager::beginPMarks() {
  ...
  if(mPmarkHandles.empty()) {
    ...
    << "&" << "pmark.appname=" << ((mTpcType == TPCR::TpcType::Pull) ? "http-put" : "http-get");
