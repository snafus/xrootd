...
#include "XrdHttpTpcR/XrdHttpTpcRTPC.hh"
...
class XrdHttpTpcRTests : public Test {};

TEST(XrdHttpTpcRTests, prepareOpenURLTest) {
  ...
  std::map<std::string,std::string> empty {};
  {
    ...
    ASSERT_EQ(resource + "?" + TPCR::TPCRHandler::OSS_TASK_OPAQUE.data(),openURL);
  }

  {
    ...
    ASSERT_TRUE(openURL.find(std::string("?") + TPCR::TPCRHandler::OSS_TASK_OPAQUE.data()) != std::string::npos);
  }
